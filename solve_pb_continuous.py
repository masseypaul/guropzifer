...
m = gp.Model("Step 1")
v = m.addMVar(shape=(22,4),vtype=gp.GRB.CONTINUOUS,name="V")
...
m.addConstrs(
    (gp.quicksum(v[i, j] for j in range(4)) ==  1 for i in range(22)), 
    name="RowSum")
...
if OBJECTIVE == "disrupt":
    diff_var = m.addVars(22,4,vtype=gp.GRB.CONTINUOUS,name="Diff")
    m.addConstrs(
        (diff_var[i,j] >= v[i,j] - m_orig[i,j] 
         for i in range(22) for j in range(4)),
        name="diff_pos"
    )
    m.addConstrs(
        (diff_var[i,j] >= m_orig[i,j] - v[i,j] 
         for i in range(22) for j in range(4)),
        name="diff_neg"
    )
    disrupt_sum = gp.quicksum(diff_var[i,j] for i in range(22) for j in range(4))
...
for j in range(4):
    sale_j = []
    for i in range(22):
        if v[i,j].x > 0.01:  # Consider assignments above 1%
            sale_j.append(i+1)
    res.append(sale_j)
...
# Print workload matrix
workload_res = [[resulting_workload[i,j].x for j in range(4)] for i in range(22)]
workload_res = np.array(workload_res)
print("Workload matrix:")
print(workload_res)

sums = [weighted_sums[i].x for i in range(4)]
print("Weighted sums:")
print(sums)

# Display final assignments with partial assignments
print("\nFinal Assignments (including partial assignments):")
for sr, bricks in enumerate(res, 1):
    print(f"SR {sr}: {bricks}")

# Display partial assignments
print("\nPartial Assignments:")
for i in range(22):
    assignments = [(j+1, v[i,j].x) for j in range(4) if v[i,j].x > 0.01]
    if len(assignments) > 1:
        print(f"Brick {i+1}: {assignments}")
