...
OBJECTIVE = "dist"  # or "disrupt"
DO_EPS = True  # Enable epsilon constraint method
...
# One SR per area
m.addConstrs(
    (gp.quicksum(v[i, j] for j in range(N_RPS)) == 1 for i in range(N_BRICKS)),
    name="RowSum"
)
...
m.addConstrs((weighted_sums[j] >= minWL for j in range(N_RPS)), name="MinWeightedSums")
m.addConstrs((weighted_sums[j] <= maxWL for j in range(N_RPS)), name="MaxWeightedSums")
...
while True:
    ...
    # Solve for distance minimization
    m.setObjective(dist_sum, gp.GRB.MINIMIZE)
    m.optimize()
    if m.status != gp.GRB.OPTIMAL:
        break
    dist_opt = dist_sum.getValue()
    ND.append([v[i, j].x for i in range(N_BRICKS) for j in range(N_RPS)])
    m.addConstr(disrupt_sum <= disrupt_sum.getValue() - eps, name="EpsilonConstraint_Disrupt")
    
    # Solve for disruption minimization
    m.setObjective(disrupt_sum, gp.GRB.MINIMIZE)
    m.optimize()
    if m.status != gp.GRB.OPTIMAL:
        break
    disrupt_opt = disrupt_sum.getValue()
    ND.append([v[i, j].x for i in range(N_BRICKS) for j in range(N_RPS)])
    m.addConstr(dist_sum <= dist_opt - eps, name="EpsilonConstraint_Dist")
